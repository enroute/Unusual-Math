...
\begin{example}
  An equilateral triangle and a regular hexagon have equal
  perimeters. If the triangle's area is 4, what is the area of the
  hexagon?
  ...
  \begin{center}
    ...
    \begin{tikzpicture}[scale=1.0]
      \begin{scope}[shift={(0,0)}]
        \draw(-30:1.1547)--(90:1.1547)--(210:1.1547)--cycle;
      \end{scope}
      % height of hexgon sqrt(3) = 1.7321
      % height of triangle 1.5r = 3/2*2/sqrt(3)=3/sqrt(3)=1.7321
      % height difference of center = 1/6 * height = 1.7321/6=0.2887
      \begin{scope}[shift={(4,0.2887)}]
...
The side length of the equilateral triangle would then be twice of the sidelength of the hexagon.
\begin{center}
  \begin{tikzpicture}
    \begin{scope}[shift={(0,0)}]
      \coordinate(A)at(-30:1.1547);
      \coordinate(B)at(90:1.1547);
      \coordinate(C)at(210:1.1547);
      \coordinate(D)at($0.5*(A)+0.5*(B)$);
      \coordinate(E)at($0.5*(B)+0.5*(C)$);
      \coordinate(F)at($0.5*(C)+0.5*(A)$);
