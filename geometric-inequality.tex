...
\begin{theorem}\label{th:cosA+cosB+cosC<=3/2}
  记三角形的三个角分别为$A,B,C$，则
  \begin{align*}
...
\begin{example}
  ...
  \begin{align*}
    a\cos A + b\cos B + c\cos C \le \frac{a+b+c}{2}
  \end{align*}
  当且仅当$\triangle ABC$是正三角形时成立。
\end{example}
\begin{proof}
  由对称性，不妨设$a\le b \le c$，从而
  \begin{align*}
    A\le B \le C, \quad \cos A \ge \cos B \ge \cos C
  \end{align*}
  首先，过顶点作三角形的高将对边分成两份，由此可得
  \begin{align*}
  ...
  \begin{center}
    \begin{tikzpicture}[scale=1.0]
      \begin{scope}
        \coordinate(B)at(0,0);\coordinate(C)at(3,0);\coordinate(A)at(2,1.5);\coordinate(D)at(2,0);
        \tkzMarkRightAngle[help lines](B,D,A)
        \tkzDrawPoints(A,B,C,D)
        \draw[dashed, help lines](A)--(D);
        \draw(A)--(B)node[midway,sloped,above]{$c$}
                --(C)node[midway,sloped,below]{$a$}
                --(A)node[midway,sloped,above]{$b$};
        \draw pic["$B$",draw=orange,<->,angle eccentricity=1.7,angle radius=.3cm] {angle=C--B--A};
        \draw pic["$C$",draw=orange,<->,angle eccentricity=1.6,angle radius=.3cm] {angle=A--C--B};
      \end{scope}
      \begin{scope}[shift={(5,0)}]
        \coordinate(B)at(0,0);\coordinate(C)at(3,0);\coordinate(A)at(5,1.5);\coordinate(D)at($(C)!(A)!(B)$);
        \tkzMarkRightAngle[help lines](B,D,A)
        \tkzDrawPoints(A,B,C,D)
        \draw[dashed, help lines](A)--(D)--(B);
        \draw(A)--(B)node[midway,sloped,above]{$c$}
                --(C)node[midway,sloped,below]{$a$}
                --(A)node[midway,sloped,below]{$b$};
        \draw pic["$B$",draw=orange,<->,angle eccentricity=2.0,angle radius=.5cm] {angle=C--B--A};
        \draw pic["$C$",draw=orange,<->,angle eccentricity=1.6,angle radius=.2cm] {angle=A--C--B};
      \end{scope}
    \end{tikzpicture}
  \end{center}
  由排序不等式，有
  \begin{align*}
    a ={}& b\cos C + c\cos B \ge b\cos B + c\cos C\\
...
\begin{proof}
  ...
  \end{align*}
  上式左边是重心的$y$坐标，右边是曲线在$x=\frac\pi3$(也是重心的$x$坐标)上的$y$坐标。本质在于$\sin$在$[0,\pi]$上是凹函数。
\end{proof}
...
\end{theorem}
\begin{proof}[提示]
  由于$A+C = 2\pi - (B+D)$，所以后两式是等价的。考虑第一个等式，其形式与余弦定理相似，可以考虑应用余弦定理。然而等式中的项的量纲是长度的4次方，而余弦定理中每项的量纲是长度的2次方，不能直接应用。将其变化为如下的等价形式：
  \begin{align*}
