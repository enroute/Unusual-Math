...
\end{example}
\begin{proof}[提示]
  由对称性，可以考虑固定一个点的情况。过此固定点作圆的直径，那么三个点组成锐角三角形等价于剩余两个点分别落在该直径的两侧。
  ...
  \begin{center}
    \begin{tikzpicture}[scale=1.0]
      ...
      \begin{scope}[shift={(5.5,0)}]
        \fill[color=red!20](0,.5)--(.5,.5)--(.5,1)--(0,1)--cycle;
        \draw[->](-1,0)--(4,0)node[right]{$x$};
        \draw[->](0,-1)--(0,2)node[above]{$y$};
        \draw[very thick](0,0)--(1,1)--(0,1)--cycle; % domain
        % \draw[dashed](-1,-.5)--(-.5,0)--(.5,1)--(1,1.5)node[right]{$y=x+\frac12$};
        \draw[dashed](-.5,-.5)--(2,2)node[right]{$y=x$};
  ...
  同样的，由对称性不妨考虑另外两个点对应的弧度满足$0<x<y<2\pi$的情况。从而$0,x,y$三个弧度对应的圆周上的点组成锐角三角形，等价于$0 < x < \pi < y < 2\pi$。由图中阴影面积可得概率为$\frac12$。
\end{proof}
