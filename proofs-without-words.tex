...
\section{无字证明}
\label{sec:proofs-without-words}
...
\begin{example}
  前$n$个数的和是$\dfrac12n(n+1)$，即
  \begin{align}
    1+2+3+\cdots+n=\frac12n(n+1)
  \end{align}
\end{example}
\begin{center}
  \begin{tikzpicture}[scale=0.6]
...
每个\hskip5pt\tikz[scale=0.5]{\fill(0,0)rectangle(1,1);\draw(0,1)rectangle(1,2) (1,0)rectangle(2,1);}\hskip5pt 中阴影部分占$\dfrac13$。
\end{example}
...
\begin{center}
  \begin{tikzpicture}[scale=1,line join=round]
    ...
    \begin{scope}[shift={(4,0)}]
      \node at (0,1) {$\xlongequal{\phantom{abcd}}$};
    \end{scope}
